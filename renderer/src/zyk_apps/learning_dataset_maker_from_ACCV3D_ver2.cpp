//difference, use HS coordinate just change center
#include <iostream>
#include <fstream>
#include <sstream>
#include <iomanip>
...
//    std::swap(ex,ey);
    ...
    if(infof.is_open())
    {
        ...
        infof<<"rotations:"<<std::endl;
        infof<<rot.at<float>(0,0)<<" "<<rot.at<float>(0,1)<<" "<<rot.at<float>(0,2)<<std::endl<<rot.at<float>(1,0)<<" "<<rot.at<float>(1,1)<<" "<<rot.at<float>(1,2)<<std::endl<<rot.at<float>(2,0)<<" "<<rot.at<float>(2,1)<<" "<<rot.at<float>(2,2)<<std::endl;
...
int main(int argc, char **argv) {
    ...
    K.at<float>(0,2)=px;
    //ATTENTION,THIS HAS TO BE TRANSFORMED
    K.at<float>(1,2)=py;
    ...
    std::string test_data_folder="/home/zyk/git/rgbd/ACCV3D_dataset_working_dir/learning_dataset2/test/Accv_" + obj_name+"/";
    std::string train_data_folder="/home/zyk/git/rgbd/ACCV3D_dataset_working_dir/learning_dataset2/training/Accv_"+obj_name + "/";
